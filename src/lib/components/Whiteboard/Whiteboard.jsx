...
const Whiteboard = ({
  ...
  onUndo = (data, event, canvas) => { },  // Callback for undoing the drawing
}) => {
...
// Assuming you have the state variables and their respective setter functions defined in your component
...
const handleUndo = () => {
  if (board && board.canvas) {
    ...
    if (objects.length > 0) {
      ...
      // Push the removed object to the redo stack
      setRedoStack([...redoStack, { type: 'ADD_OBJECT', object: removedObject }]);
      board.canvas.remove(removedObject);
      onUndo();
    }
  }
};
...
const handleRedo = () => {
  if (redoStack.length > 0) {
    ...
    applyRedoAction(lastRedoAction);
  }
};

const applyRedoAction = (action) => {
      ...
      <ToolbarHolderS>
        ...
        <ToolbarS>
          ...
          {!!enabledControls.REDO && (
            <ToolbarItemS>
              <ButtonS onClick={handleRedo} title="Redo">
                Redo
              </ButtonS>
            </ToolbarItemS>
          )}
